...
/// Class ArchiveType utilse pour la liste déroulante
class PluginRtntestalexArchiveType extends CommonDropdown {

    static $rightname = PluginRtntestalexProfile::RIGHT_RTNTESTALEX_ARCHIVETYPE;
    ...
    static function getTypeName($nb = 0) {
        global $LANG;
        return __s('gen_type_archives', 'rtntestalex');
    }
    ...
    static function install(Migration $migration) {
        ...
        if (!TableExists($table)) {
            ...
            $query = "INSERT INTO `$table`(`name`) VALUES ('Archivage mensuel')";
            $DB->query($query) or die("Error adding in table $table (1)");
            $query = "INSERT INTO `$table`(`name`) VALUES ('Archivage annuel')";
            $DB->query($query) or die("Error adding in table $table (2)");
            $query = "INSERT INTO `$table`(`name`) VALUES ('Archivage mensuel AS/400')";
            $DB->query($query) or die("Error adding in table $table (3)");
            $query = "INSERT INTO `$table`(`name`) VALUES ('Archivage annuel AS/400')";
    ...
    static function uninstall() {
        ...
        // Supression de l'intitulé dans la partie intitulé
        $dropdownTranslation = new DropdownTranslation();
        $dropdownTranslation->deleteByCriteria(array("itemtype = 'PluginRtntestalexArchiveType'"), 1);
    ...
    static function transfer($ID, $entity) {
        ...
        $rtnarchiveType = new self();

        if ($ID > 0) {
          //Mode Ajout
            $query = "SELECT *
                   FROM `" . $rtnarchiveType->getTable() . "`
                   WHERE `id` = '$ID'";
            ...
            if ($result = $DB->query($query)) {
                if ($DB->numrows($result)) {
                    ...
                    $newID = $rtnarchiveType->getID($input);

                    if ($newID < 0) {
                        $newID = $rtnarchiveType->import($input);
                    }
                    return $newID;
                }
            }
    ...
    **/
    function getSearchOptions() {
        ...
        $tab = array();
        $tab['common'] = __s('Type of archives', 'rtntestalex');
